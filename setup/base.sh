...
# This script installs Agent OS to the system at ~/.agent-os
...
# Set installation directory to ~/.agent-os
HOME_DIR="$HOME"
INSTALL_DIR="$HOME_DIR/.agent-os"

echo "📍 Agent OS will be installed to: $INSTALL_DIR"
echo ""
...
echo ""
echo "📦 Installing the latest version of Agent OS from the GitHub repository..."
...
if [[ -n "$SHELL_RC" ]]; then
    ...
    if ! grep -q "alias setup-aos=" "$SHELL_RC" 2>/dev/null; then
        echo "" >> "$SHELL_RC"
        echo "# Agent OS aliases" >> "$SHELL_RC"
        echo "alias setup-aos='$INSTALL_DIR/setup/project.sh'" >> "$SHELL_RC"
        echo "  ✓ Added setup-aos alias to $SHELL_RC"
    else
        echo "  ✓ setup-aos alias already exists in $SHELL_RC"
    fi
else
    echo "  ⚠️  Could not detect shell configuration file. Please manually add:"
    echo "     alias setup-aos='$INSTALL_DIR/setup/project.sh'"
fi
...
echo "Next steps:"
echo ""
echo "1. Reload your shell or run: source $SHELL_RC"
echo ""
echo "2. Customize your standards in $INSTALL_DIR/standards/"
echo ""
echo "3. Configure project types in $INSTALL_DIR/config.yml"
echo ""
echo "4. Navigate to a project directory and run: setup-aos"
echo ""
echo "--------------------------------"
echo ""
